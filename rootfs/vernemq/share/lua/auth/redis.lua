...
function auth_on_register(reg)
  password = obf.decrypt(reg.password)
  if reg.username ~= nil and password ~= nil and reg.client_id ~= nil then
    key = "client:" .. reg.username ..":" .. reg.client_id
    res = redis.cmd(pool, "GET " .. key)
    if res then
      res = json.decode(res)
      if res.passhash == bcrypt.hashpw(password, res.passhash) then
        cache_insert(
          res.mountpoint,
          reg.client_id,
          reg.username,
